...
def main():
    ...
    if training_args.do_step_schedule_per_epoch:
        ...
        disc_cos_sl = torch.optim.lr_scheduler.CosineAnnealingLR(
            disc_optimizer, T_max=num_training_steps-num_warmups_steps, eta_min=1e-6, last_epoch=-1
        )
        ...
        dur_disc_warmup_sl = torch.optim.lr_scheduler.LinearLR(
            dur_disc_optimizer, start_factor=1e-3, end_factor=1.0, total_iters=num_warmups_steps
        )
        dur_disc_cos_sl = torch.optim.lr_scheduler.CosineAnnealingLR(
            dur_disc_optimizer, T_max=num_training_steps-num_warmups_steps, eta_min=1e-6, last_epoch=-1
        )
        dur_disc_lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
            dur_disc_optimizer, schedulers=[dur_disc_warmup_sl, dur_disc_cos_sl], milestones=[num_warmups_steps]
        )
